import torch
from .res_multistep import sample_step_res_multistep, sample_step_res_multistep_official
...
def sample_step_res_multistep_ancestral(
    ...
    - When official_comfy is True, use the official EPS ancestral update
      (no alpha mixing; additive noise after integrating to sigma_down).
    - Otherwise, use the res4lyf ancestral path (alpha_ratio mixing).

    Returns (x, learning_ratio, prev_sigma_down_like) for engine chaining.
    """
    if bool(official_comfy):
        return sample_step_res_multistep_official(
            model=model,
